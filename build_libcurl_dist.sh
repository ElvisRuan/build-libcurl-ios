...
#if LIBCURL_VERSION_MAJOR < 8 || LIBCURL_VERSION_MINOR < 11
...
function build_for_arch() {
  ...
  ./configure --disable-shared --without-zlib --enable-static --enable-ipv6 ${SSL_FLAG} --without-libpsl --without-libidn2 --without-nghttp2 --host="${HOST}" --prefix=${PREFIX} && make -j8 && make install
}
...
then
  ...
  check_curl_ver
  # From version 7.77.0 --with-darwinssl no longer works, use --with-secure-transport for instead.
  export SSL_FLAG=--with-secure-transport
fi

TMP_DIR=/Users/appiron/Downloads/build_libcurl_$$
...
mkdir -p ${TMP_DIR}/lib/
${DEVROOT}/usr/bin/lipo -create \
  ${TMP_DIR}/x86_64/lib/libcurl.a \
  ${TMP_DIR}/arm64/lib/libcurl.a \
  -output ${TMP_DIR}/lib/libcurl.a 
